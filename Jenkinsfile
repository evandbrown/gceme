node('docker') {
  ...
  docker.image('google/cloud-sdk').inside {
    ...
    sh('wget -v https://storage.googleapis.com/kubernetes-release/release/v1.0.1/bin/linux/amd64/kubectl > wget.txt 2>&1 ; mv kubectl /usr/bin/kubectl ; chmod +x /usr/bin/kubectl')
